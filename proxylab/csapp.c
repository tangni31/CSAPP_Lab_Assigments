...
int Rio_writen(int fd, void *usrbuf, size_t n) 
{
	...
	if(errno == EPIPE) return 2;/*client closed connection Hints 4*/
...
ssize_t Rio_readnb(rio_t *rp, void *usrbuf, size_t n, int *serverstat) 
{
    ssize_t rc;
	...
	rc = 0;
	if(errno == ECONNRESET) *serverstat = 0;
    }
	
    return rc;
}

ssize_t Rio_readlineb(rio_t *rp, void *usrbuf, size_t maxlen, int *serverstat) 
{
    ssize_t rc;
